...
`
  ...
  function lcs(a, b, eq) {
    ...
    for (let i2 = 0; i2 <= m; i2++) {
      ...
      for (let j2 = 0; j2 <= n; j2++) {
        if (i2 === 0 || j2 === 0) dp[i2][j2] = 0;
        else if (eq(a[i2 - 1], b[j2 - 1])) dp[i2][j2] = dp[i2 - 1][j2 - 1] + 1;
  ...
  var mode_toggle_default = {
    ...
    render() {
      ...
      this.el.innerHTML = MODES.map(
        ({ id, label, icon, title }) => `<button type="button" class="mode-btn ${id === current ? "mode-btn--active" : ""}" data-mode="${id}" title="${title}"><span class="mode-icon">${icon}</span> ${label}</button>`
      ).join("");
    },
    onClick(e) {
  ...
  var diff_view_default = {
    ...
    acceptBlock(el) {
      if (el.classList.contains("inkwell-diff-removed")) {
        ...
        el.querySelectorAll(".inkwell-diff-word-added").forEach(
          (s) => s.replaceWith(document.createTextNode(s.textContent))
        );
  ...
  // js/hooks/doc_map.js
  var SHEET_SELECTOR = "#doc-map-sheet";
  var BACKDROP_SELECTOR = "#doc-map-backdrop";
  var doc_map_default = {
    mounted() {
  ...
  // js/hooks/doc_rail_nav.js
  var doc_rail_nav_default = {
    mounted() {
      this.handler = (e) => {
  ...
  // js/hooks/find_bar.js
  var MAX_MATCHES = 500;
  var SKIP_TAGS = /* @__PURE__ */ new Set(["PRE", "CODE", "SCRIPT", "STYLE", "NOSCRIPT"]);
  var DEBOUNCE_MS = 120;
  ...
  var find_bar_default = {
    ...
    navigate(direction) {
      ...
      const next = (this.activeIndex + direction + this.matches.length) % this.matches.length;
    ...
    updateCounter() {
      ...
      this.counter.textContent = this.matches.length >= MAX_MATCHES ? `${prefix}+` : prefix;
    }
  };
  ...
  var mermaid_default = {
    ...
    runMermaid() {
      ...
      if (!window.__inkwell_mermaid_initialized) {
        const theme = document.querySelector("[data-theme]")?.dataset?.theme === "light" ? "default" : "dark";
  ...
  // js/hooks/picker_keys.js
  var NAV_KEYS = /* @__PURE__ */ new Set(["ArrowUp", "ArrowDown", "Enter", "Escape"]);
  var picker_keys_default = {
    mounted() {
      this.handler = (e) => {
  ...
  // js/hooks/scrollspy.js
  var HEADING_SELECTOR = "#page-ctn h1, #page-ctn h2, #page-ctn h3, #page-ctn h4";
  var scrollspy_default = {
    ...
    markActive(id) {
      document.querySelectorAll("#doc-rail .doc-rail-link, #doc-map-content .doc-rail-link").forEach((a) => a.classList.toggle("doc-rail-active", a.dataset.target === id));
    }
  };
  ...
  function build() {
    ...
    modal.innerHTML = '<div id="zoom-modal-backdrop"></div><div id="zoom-modal-dialog" role="dialog" aria-modal="true" aria-label="Zoomed content"><div id="zoom-modal-toolbar"><div id="zoom-modal-title"></div><div id="zoom-modal-actions"><button type="button" class="zoom-modal-btn" data-action="zoom-in" aria-label="Zoom in">+</button><button type="button" class="zoom-modal-btn" data-action="zoom-out" aria-label="Zoom out">\u2212</button><button type="button" class="zoom-modal-btn zoom-modal-btn-reset" data-action="reset">Reset</button><button type="button" class="zoom-modal-btn zoom-modal-btn-close" data-action="close" aria-label="Close">\xD7</button></div></div><div id="zoom-modal-viewport"><div id="zoom-modal-canvas"></div></div></div>';
  ...
  function openForMermaid(preEl) {
    ...
    clone2.removeAttribute("width");
    clone2.removeAttribute("height");
  ...
  function openForImage(imgEl) {
    const clone2 = document.createElement("img");
    clone2.src = imgEl.currentSrc || imgEl.src;
    clone2.alt = imgEl.alt || "";
    clone2.decoding = "async";
    clone2.style.width = "100%";
    clone2.style.height = "100%";
    clone2.style.display = "block";
    clone2.style.objectFit = "contain";
    const finish = () => {
      open({
        type: "image",
        ...
        source: imgEl,
        node: clone2,
        width: clone2.naturalWidth || imgEl.naturalWidth || imgEl.width || 1,
        height: clone2.naturalHeight || imgEl.naturalHeight || imgEl.height || 1
      });
    };
    ...
    clone2.addEventListener("load", finish, { once: true });
    clone2.addEventListener("error", finish, { once: true });
  }
  function open(config) {
    ...
    State.triggerElement = document.activeElement instanceof HTMLElement ? document.activeElement : null;
  ...
  function close() {
    ...
    if (State.pointerId !== null && State.viewport.hasPointerCapture && State.viewport.hasPointerCapture(State.pointerId)) {
  ...
  // js/app.js
  var Hooks2 = {
    DiffView: diff_view_default,
    DocMap: doc_map_default,
    DocRailNav: doc_rail_nav_default,
    FindBar: find_bar_default,
    Mermaid: mermaid_default,
    ModeToggle: mode_toggle_default,
    PickerKeys: picker_keys_default,
    PickerOverlay: picker_overlay_default,
